...
const STORAGE_ROOT = path.join(process.cwd(), "storage", "patient-documents");
...
export async function storeHistoryDocument(
  historyId: number,
  extension: string,
  bytes: Uint8Array,
): Promise<string> {
...
export async function readHistoryDocument(key: string): Promise<Buffer> {
  return readFile(resolveStoragePath(key));
}

export async function removeHistoryDocument(key: string): Promise<void> {
  try {
